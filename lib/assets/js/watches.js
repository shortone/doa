...
var WatchView = Backbone.Marionette.ItemView.extend({
  ...
  updateStatus : function() {
    ...
    if (this.model.get('lastSeenAt')) {
      status = status.replace(/\%\{time\}/, moment(new Date(this.model.get('lastSeenAt'))).fromNow(true));
    }
  ...
  updateLastSeenAt : function() {
    if (!this.model.get('lastSeenAt')) {
      this.ui.lastSeenAt.text(Translations.pingInstructions);
    } else {
      this.ui.lastSeenAt.text(Translations.lastPing.replace(/\%\{time\}/, moment(new Date(this.model.get('lastSeenAt'))).format('dddd, MMMM Do YYYY, h:mm:ss a')));
    }
  }
});
